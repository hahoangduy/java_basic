public class bai4 {
    public static void main(String[] args) {

        // kiem tra kieu int
        System.out.println("max cua kieu int " + Integer.MAX_VALUE);
        System.out.println("min cua kieu int " + Integer.MIN_VALUE);


        // kiem tra kieu byte
        System.out.println("max cua kieu byte " + Byte.MAX_VALUE);
        System.out.println("min cua kieu byte " + Byte.MIN_VALUE);


        // kiem tra kieu short
        System.out.println("max cua kieu short " + Short.MAX_VALUE);
        System.out.println("min cua kieu short " + Short.MIN_VALUE);


        // kiem tra kieu long
        System.out.println("max cua kieu long " + Long.MAX_VALUE);
        System.out.println("min cua kieu long " + Long.MIN_VALUE);

    }
}
